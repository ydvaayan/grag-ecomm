...
collections_seed = [
    (
        "C001",
        "Fathers Day Gifts",
        "Curated gifts for dads across hobbies",
        "Summer",
        "Men",
    ),
    ...
    (
        "C010",
        "Monsoon Must-Haves",
        "Quick-dry, umbrellas, organizers",
        "Monsoon",
        "All",
    ),
...
def pick_tags(cat, sub):
    ...
    if cat in ["Women Ethnic", "Women Western", "Jewellery", "Footwear"]:
        pool.update(
            ["women", "fashion", "style", "ethnic", "western", "party", "wedding"]
        )
    if cat in ["Men"]:
...
def assign_collections(p):
    ...
    if "gift" in tags and (
        ...
        or cat == "Men"
        or "gadgets" in tags
        or "wallet" in p["title"].lower()
    ):
        cols.append("C001")
    if "gift" in tags and (
        "women" in tags
        or "beauty" in tags
        or cat in ["Women Western", "Women Ethnic", "Jewellery"]
    ):
        cols.append("C002")
...
write_csv(OUT / "sellers.csv", ["seller_id", "name", "region", "rating"], sellers)
...
write_csv(OUT / "collection_items.csv", ["collection_id", "item_id"], collection_items)
...
print(
    "Generated: products.csv (1000), sellers.csv (15), collections.csv (25), collection_items.csv (~{}), users.csv (10)".format(
